...
export async function GET(request: NextRequest) {
  ...
  const input={identityVerified:String(row.identity_status||"")==="source_verified",verifiedContactCount:Number(row.verified_contact_count||0),contactResearchStatus:row.contact_research_status?String(row.contact_research_status):null};
