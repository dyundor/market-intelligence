...
export async function GET(request:NextRequest){
  ...
  if(market!=="美国"||flow!=="进口")return NextResponse.json({available:false,reason:"当前企业级 ImportYeti 数据仅覆盖美国海运进口。",suppliers:[]});
  const hs=HS[product]||"";const keyword=KEYWORDS[product]||"";const hsLike=`%${hs}%`;const keywordLike=`%${keyword}%`;
  ...
  const importerSql=`WITH matching_rel AS (
    ...
    ) SELECT i.id,i.name,i.address,i.country,i.country_code,i.admin1_code,i.admin1_name,i.city_name,i.location_names,i.location_precision,i.website,i.total_shipments,i.source_url,COALESCE(ac.supplier_count,0) supplier_count,SUM(r.shipment_count) relationship_shipments,GROUP_CONCAT(DISTINCT s.name) suppliers,GROUP_CONCAT(DISTINCT r.product_descriptions) products,
  ...
  const [result,importers,coverage]=await Promise.all([
    env.DB.prepare(sql).bind(hsLike,keywordLike,...months).all(),
    env.DB.prepare(importerSql).bind(hsLike,keywordLike,...months).all(),
    env.DB.prepare(coverageSql).bind(product).all(),
  ]);
  const latestAvailableMonth=String((coverage.results[0] as {month?:string}|undefined)?.month||"");
  return NextResponse.json({available:true,dataset:"importyeti_free_web",market,flow,product,hsCode:hs.replace(".",""),requestedMonths:months,latestAvailableMonth,importers:importers.results,suppliers:result.results,storedShipmentCoverage:coverage.results});
}
