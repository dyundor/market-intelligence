...
import { computeOpportunityMetrics } from "../../../lib/leads/opportunity-pipeline.ts";

export async function GET() {
  if (!env.DB) return NextResponse.json({ error: "Database unavailable" }, { status: 503 });
  const rows = await env.DB.prepare(
    `SELECT w.company_id, w.lead_status,w.opportunity_value_usd,w.opportunity_probability,
