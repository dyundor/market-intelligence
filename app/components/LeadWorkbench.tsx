...
import { contactRouteQuality } from "../../lib/leads/contact-quality.ts";
...
export type WatchlistItem = {
  ...
  targetMarket:string|null; requiredCertifications:string|null; estimatedAnnualUnits:number|null;
  targetMoq:number|null; quoteRequirements:string|null;
...
type Dashboard = {metrics:{totalLeads:number;overdue:number;dueToday:number;contacted:number;positive:number;positiveRate:number;opportunityCount:number;pipelineValueUsd:number;weightedPipelineValueUsd:number;quoteRequested:number;quoteSent:number;awaitingQuote:number;openQuotes:number;won:number;lost:number;requestToQuoteRate:number;quoteWinRate:number;missingDecisionMaker:number};tasks:Array<{companyId:string;companyName:string;nextAction:string|null;nextActionDue:string|null;timing:"overdue"|"today"|"unscheduled"|"upcoming";opportunityValueUsd:number|null;weightedValueUsd:number;expectedCloseDate:string|null}>;contactGaps:Array<{companyId:string;companyName:string;priorityScore:number;bestContactRouteQuality:string|null;bestContactLabel:string|null;recommendedAction:string}>;today:string};
...
type OutreachReadiness = {ready:boolean;blockers:Array<"identity_unverified"|"verified_contact_missing"|"contact_research_unresolved"|"lead_disqualified">;identityVerified:boolean;verifiedContactCount:number;contactResearchStatus:string|null;bestContactRouteQuality:"decision_maker"|"business_route"|"general_route"|"fallback"|null;contactRouteGuidance:string|null};
...
export function LeadWorkbench({items,loading,locale,onUpdate,onRemove,onOpenCompany}:{items:WatchlistItem[];loading:boolean;locale:string;onUpdate:(id:string,changes:Record<string,string|number>)=>Promise<void>;onRemove:(id:string)=>Promise<void>;onOpenCompany:(id:string)=>void}){
  ...
  const selectedQuoteReadiness=selected?quoteReadiness(selected):null;
  ...
  async function generateDraft(requestedPurpose:"initial"|"follow_up"|unknown="initial"){
    const purpose=requestedPurpose==="follow_up"?"follow_up":"initial";
    if(!selected)return;
    const verifiedContact=contacts.find(contact=>contact.verificationStatus==="verified"&&contact.label)?.label;
    const response=await fetch("/api/lead-drafts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({companyId:selected.companyId,contactName:verifiedContact,purpose})});
    if(!response.ok){setMessage(purpose==="follow_up"?(zh?"最近结果不适合生成跟进邮件":"Latest outcome is not eligible for a follow-up draft"):(zh?"外联草稿生成失败":"Draft could not be generated"));return;}
    const draft=await response.json() as Draft;setDrafts(previous=>[draft,...previous]);setMessage(purpose==="follow_up"?(zh?"跟进草稿已生成，请结合最新沟通审核":"Follow-up draft generated — review against the latest conversation"):(zh?"外联草稿已生成，请审核后再使用":"Draft generated — review before use"));
  }
  ...
  async function copyQuoteBrief(){
    if(!selected)return;
    const response=await fetch(`/api/quote-brief?companyId=${encodeURIComponent(selected.companyId)}`);
    const payload=await response.json().catch(()=>null) as {text?:string;missing?:string[]}|null;
    if(!response.ok||!payload?.text){setMessage(zh?`报价资料尚未齐全${payload?.missing?.length?`（缺 ${payload.missing.length} 项）`:""}`:"Quote qualification is incomplete");return;}
    try{await navigator.clipboard.writeText(payload.text);setMessage(zh?"内部报价简报已复制，请交报价团队审核":"Internal quote handoff copied — send it for costing review");}
    catch{setMessage(zh?"报价简报已生成，但无法自动复制":"Quote handoff generated but could not be copied");}
  }
  ...
  return <div className="lead-workbench" aria-busy={loading||detailLoading}>
    {dashboard?<><div className="lead-dashboard"><span><b>{dashboard.metrics.totalLeads}</b>{zh?"全部 Lead":"Total leads"}</span><span className={dashboard.metrics.overdue?"alert":""}><b>{dashboard.metrics.overdue}</b>{zh?"逾期跟进":"Overdue"}</span><span><b>{dashboard.metrics.dueToday}</b>{zh?"今日待办":"Due today"}</span><span><b>{dashboard.metrics.contacted}</b>{zh?"已触达":"Contacted"}</span><span className={dashboard.metrics.missingDecisionMaker?"alert":""}><b>{dashboard.metrics.missingDecisionMaker}</b>{zh?"待找采购负责人":"Need decision owner"}</span><span><b>{dashboard.metrics.positiveRate}%</b>{zh?"积极反馈率":"Positive rate"}</span><span><b>{new Intl.NumberFormat(locale,{style:"currency",currency:"USD",maximumFractionDigits:0}).format(dashboard.metrics.pipelineValueUsd)}</b>{zh?`管道总额（${dashboard.metrics.opportunityCount}）`:`Pipeline (${dashboard.metrics.opportunityCount})`}</span><span><b>{new Intl.NumberFormat(locale,{style:"currency",currency:"USD",maximumFractionDigits:0}).format(dashboard.metrics.weightedPipelineValueUsd)}</b>{zh?"加权管道":"Weighted pipeline"}</span><span className={dashboard.metrics.awaitingQuote?"alert":""}><b>{dashboard.metrics.awaitingQuote}</b>{zh?"待出报价":"Awaiting quote"}</span><span><b>{dashboard.metrics.openQuotes}</b>{zh?"已报价待决策":"Open quotes"}</span><span><b>{dashboard.metrics.quoteWinRate}%</b>{zh?`报价赢单率（${dashboard.metrics.won}/${dashboard.metrics.quoteSent}）`:`Quote win rate (${dashboard.metrics.won}/${dashboard.metrics.quoteSent})`}</span><a href="/api/leads/export" download>{zh?"导出销售执行包":"Export sales execution pack"}</a></div>{dashboard.tasks.length?<div className="lead-tasks"><strong>{zh?`跟进待办（${dashboard.tasks.length}）`:`Follow-up queue (${dashboard.tasks.length})`}</strong>{dashboard.tasks.map(task=><button key={`${task.companyId}-${task.nextActionDue||"unscheduled"}`} className={task.timing} onClick={()=>setSelectedId(items.find(item=>item.companyId===task.companyId)?.id||null)}><span>{task.companyName}{task.opportunityValueUsd?<small>{new Intl.NumberFormat(locale,{style:"currency",currency:"USD",maximumFractionDigits:0}).format(task.opportunityValueUsd)} · {zh?"加权":"weighted"} {new Intl.NumberFormat(locale,{style:"currency",currency:"USD",maximumFractionDigits:0}).format(task.weightedValueUsd)}</small>:null}</span><b>{task.timing==="unscheduled"?(zh?"商机缺少下一步":"Opportunity has no next action"):(task.nextAction|| (zh?"待跟进":"Follow up"))}</b><em>{task.nextActionDue||(task.expectedCloseDate?`${zh?"预计成交":"Close"} ${task.expectedCloseDate}`:(zh?"请安排日期":"Schedule date"))}</em></button>)}</div>:null}{dashboard.contactGaps.length?<div className="contact-gap-queue"><strong>{zh?`采购负责人缺口（${dashboard.contactGaps.length}）`:`Decision-owner gaps (${dashboard.contactGaps.length})`}</strong>{dashboard.contactGaps.slice(0,8).map(gap=><button key={gap.companyId} onClick={()=>setSelectedId(items.find(item=>item.companyId===gap.companyId)?.id||null)}><b>{gap.priorityScore}</b><span><strong>{gap.companyName}</strong><small>{gap.bestContactLabel|| (zh?"仅有电话兜底":"Fallback route only")}</small></span><em>{gap.recommendedAction}</em></button>)}</div>:null}</>:null}
    ...
    <div className="lead-layout">
      ...
      {selected?<section className="lead-detail">
        ...
        <div className="quote-qualification"><header><strong>{zh?"报价准备资料":"Quote qualification"}</strong><span className={selectedQuoteReadiness?.ready?"ready":"incomplete"}>{selectedQuoteReadiness?.ready?(zh?"资料齐全":"Ready"):(zh?`待补 ${selectedQuoteReadiness?.missing.length||0} 项`:`${selectedQuoteReadiness?.missing.length||0} missing`)}</span></header><div><label>{zh?"目标市场":"Target market"}<input key={`market-${selected.id}-${selected.targetMarket||""}`} defaultValue={selected.targetMarket||""} onBlur={event=>{if(event.target.value!==selected.targetMarket)onUpdate(selected.id,{targetMarket:event.target.value})}}/></label><label>{zh?"认证要求":"Required certifications"}<input key={`cert-${selected.id}-${selected.requiredCertifications||""}`} defaultValue={selected.requiredCertifications||""} placeholder="cUPC / NSF / WaterSense" onBlur={event=>{if(event.target.value!==selected.requiredCertifications)onUpdate(selected.id,{requiredCertifications:event.target.value})}}/></label><label>{zh?"预计年需求量":"Estimated annual units"}<input key={`annual-${selected.id}-${selected.estimatedAnnualUnits??""}`} type="number" min="0" step="1" defaultValue={selected.estimatedAnnualUnits??""} onBlur={event=>{const value=event.target.valueAsNumber;if(Number.isSafeInteger(value)&&value>=0&&value!==selected.estimatedAnnualUnits)onUpdate(selected.id,{estimatedAnnualUnits:value})}}/></label><label>{zh?"目标 MOQ":"Target MOQ"}<input key={`moq-${selected.id}-${selected.targetMoq??""}`} type="number" min="0" step="1" defaultValue={selected.targetMoq??""} onBlur={event=>{const value=event.target.valueAsNumber;if(Number.isSafeInteger(value)&&value>=0&&value!==selected.targetMoq)onUpdate(selected.id,{targetMoq:value})}}/></label><label className="quote-requirements">{zh?"规格、表面与包装要求":"Specifications, finishes & packaging"}<input key={`requirements-${selected.id}-${selected.quoteRequirements||""}`} defaultValue={selected.quoteRequirements||""} onBlur={event=>{if(event.target.value!==selected.quoteRequirements)onUpdate(selected.id,{quoteRequirements:event.target.value})}}/></label></div>{selectedQuoteReadiness?.ready?<button className="quote-brief-action" onClick={copyQuoteBrief}>{zh?"复制内部报价简报":"Copy internal quote handoff"}</button>:null}</div>
        {message?<p className="lead-message">{message}</p>:null}
        {readiness?<div className={`outreach-readiness ${readiness.ready?"ready":"blocked"}`}><strong>{readiness.ready?(zh?"可安全审批外联":"Ready for outreach approval"):(zh?"外联审批已阻止":"Outreach approval blocked")}</strong><span>{readiness.ready?(readiness.bestContactRouteQuality==="decision_maker"?(zh?"已验证采购或产品负责人路线":"Verified purchasing or product-owner route"):readiness.bestContactRouteQuality==="business_route"?(zh?"已验证业务路线；联系时要求转采购负责人":"Verified business route — ask for the purchasing owner"):readiness.bestContactRouteQuality==="general_route"?(zh?"当前为通用路线；先要求内部转采购部门":"General route — request routing to purchasing"):(zh?"当前仅有备用路线；先电话确认采购负责人":"Fallback route — identify the purchasing owner first")):readiness.blockers.map(blocker=>blocker==="identity_unverified"?(zh?"公司身份未核验":"Identity unverified"):blocker==="verified_contact_missing"?(zh?"缺少已验证联系方式":"Verified contact missing"):blocker==="lead_disqualified"?(zh?"已确认为非目标客户":"Lead disqualified"):(zh?"公司研究尚未解析":"Contact research unresolved")).join(" · ")}</span></div>:null}
