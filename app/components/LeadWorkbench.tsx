...
type OutreachReadiness = {ready:boolean;blockers:Array<"identity_unverified"|"verified_contact_missing"|"contact_research_unresolved">;identityVerified:boolean;verifiedContactCount:number;contactResearchStatus:string|null};
...
export function LeadWorkbench({items,loading,locale,onUpdate,onRemove,onOpenCompany}:{items:WatchlistItem[];loading:boolean;locale:string;onUpdate:(id:string,changes:Record<string,string|number>)=>Promise<void>;onRemove:(id:string)=>Promise<void>;onOpenCompany:(id:string)=>void}){
  ...
  return <div className="lead-workbench" aria-busy={loading||detailLoading}>
    {dashboard?<><div className="lead-dashboard"><span><b>{dashboard.metrics.totalLeads}</b>{zh?"全部 Lead":"Total leads"}</span><span className={dashboard.metrics.overdue?"alert":""}><b>{dashboard.metrics.overdue}</b>{zh?"逾期跟进":"Overdue"}</span><span><b>{dashboard.metrics.dueToday}</b>{zh?"今日待办":"Due today"}</span><span><b>{dashboard.metrics.contacted}</b>{zh?"已触达":"Contacted"}</span><span><b>{dashboard.metrics.positiveRate}%</b>{zh?"积极反馈率":"Positive rate"}</span><a href="/api/leads/export" download>{zh?"导出销售执行包":"Export sales execution pack"}</a></div>{dashboard.tasks.length?<div className="lead-tasks"><strong>{zh?`跟进待办（${dashboard.tasks.length}）`:`Follow-up queue (${dashboard.tasks.length})`}</strong>{dashboard.tasks.map(task=><button key={`${task.companyId}-${task.nextActionDue}`} className={task.timing} onClick={()=>setSelectedId(items.find(item=>item.companyId===task.companyId)?.id||null)}><span>{task.companyName}</span><b>{task.nextAction|| (zh?"待跟进":"Follow up")}</b><em>{task.nextActionDue}</em></button>)}</div>:null}</>:null}
