...
export class ImportYetiWebProvider implements Provider {
  ...
  async fetch(query: QueryRequest): Promise<unknown> {
    ...
    const sql = `WITH matching_rel AS (
        ...
        FROM importyeti_web_relationships r JOIN importyeti_web_entities i ON i.id=r.importer_id AND i.country='United States' AND i.id NOT LIKE 'seed-%'
      ...
      FROM matching_rel r JOIN importyeti_web_entities s ON s.id=r.supplier_id AND s.id NOT LIKE 'seed-%' LEFT JOIN month_shipments m ON m.supplier_id=s.id
    ...
    const importerSql = `WITH matching_rel AS (
      ...
      FROM matching_rel r JOIN importyeti_web_entities i ON i.id=r.importer_id AND i.country='United States' AND i.id NOT LIKE 'seed-%' JOIN importyeti_web_entities s ON s.id=r.supplier_id AND s.id NOT LIKE 'seed-%' LEFT JOIN all_counts ac ON ac.importer_id=i.id
